<template>
  <div class="auth-clientid-table">
      ...
      <el-form ref="record" :model="record" :rules="rules" @keyup.enter.native="save">
        <el-col :span="6">
          <el-form-item prop="clientid">
            <el-input v-model="record.clientid" placeholder="Client ID" class="input-radius"> </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
        ...
        <el-col :span="2">
          <el-button class="add" round plain type="success" icon="el-icon-plus" size="medium" @click="save">
            {{ $t('oper.add') }}
          </el-button>
        </el-col>
      </el-form>
    ...
    <el-table border :data="tableData">
      ...
      <el-table-column width="120px" :label="$t('oper.oper')">
        <template slot-scope="{ row, $index, _self }">
          <el-button size="mini" type="success" plain @click="showEdit(row)">
            {{ $t('oper.edit') }}
          </el-button>
          <el-popover :ref="`popover-${$index}`" placement="right" trigger="click">
            ...
            <div style="text-align: right;">
              ...
              <el-button size="mini" type="success" @click="handleDelete(row, $index, _self)">
                {{ $t('oper.confirm') }}
              </el-button>
            </div>
    ...
    <el-dialog :visible.sync="editVisible" width="400px" :title="$t('oper.edit')" @keyup.enter.native="handleEdit">
      <el-form class="el-form--public app-info" ref="editRecord" size="medium" :rules="rules" :model="editRecord">
        <el-row :gutter="20">
          <el-col :span="24">
      ...
      <div slot="footer">
        <el-button type="text" class="cache-btn" @click="editVisible = false">
          {{ $t('oper.cancel') }}
        </el-button>
        <el-button
          type="success"
...
<script>
import { Dialog, Input, Button, Table, TableColumn, Form, FormItem, Row, Col } from 'element-ui'

export default {
  name: 'auth-clientid-table',
  components: {
    'el-dialog': Dialog,
    'el-input': Input,
    'el-button': Button,
  ...
  data() {
    return {
      ...
      record: {},
      rules: {
        clientid: { required: true, message: this.$t('plugins.clientidRequired') },
  ...
  methods: {
    ...
    save() {
      this.$refs.record.validate(valid => {
        ...
        this.$httpPost('/auth_clientid', data)
          .then(() => {
            this.loadData()
    ...
    handleDelete(row, index, self) {
      this.$httpDelete(`/auth_clientid/${row.clientid}`)
        .then(() => {
          this.loadData()
          // Close popover
    ...
    showEdit(row) {
      ...
      this.$httpGet(`/auth_clientid/${row.clientid}`)
        .then(response => {
    ...
    handleEdit() {
      this.$refs.editRecord.validate(valid => {
        ...
        this.$httpPut(`/auth_clientid/${this.editRecord.clientid}`, data)
          .then(() => {
            this.loadData()
