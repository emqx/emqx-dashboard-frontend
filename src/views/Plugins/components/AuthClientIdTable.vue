      ...
      <el-form ref="record" :model="record" :rules="rules" @keyup.enter.native="save">
        <el-col :span="5">
          <el-form-item prop="clientid">
            <el-input size="small" v-model="record.clientid" placeholder="Client ID">
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="5">
          <el-form-item prop="password">
            <el-input v-model="record.password" type="password" size="small" placeholder="password">
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="2">
...
export default {
  ...
  methods: {
    loadData() {
      this.$httpGet('/auth_clientid').then((response) => {
    ...
    save() {
      this.$refs.record.validate((valid) => {
        ...
        this.$httpPost('/auth_clientid', data).then(() => {
          this.loadData()
    ...
    handleDelete(row, index, self) {
      this.$httpDelete(`/auth_clientid/${row.clientid}`).then(() => {
        this.loadData()
        // Close popover
    ...
    showEdit(row) {
      ...
      this.$httpGet(`/auth_clientid/${row.clientid}`).then((response) => {
    ...
    handleEdit() {
      this.$refs.editRecord.validate((valid) => {
        ...
        this.$httpPut(`/auth_clientid/${this.editRecord.clientid}`, data).then(() => {
          this.loadData()
